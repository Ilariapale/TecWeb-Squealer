...
module.exports = {
  ...
  startAutomaticPost: async () => {
    const cronWeather = cron.schedule("5 * * * *", async () => {
      await postWeatherSqueals();
    });
    const cronRandom = cron.schedule("20 * * * *", async () => {
      await postRandomSqueals();
    });
    const cronControversial = cron.schedule("35 * * * *", async () => {
      await postControversialSqueals();
    });
  ...
  postControversialSqueals: async () => {
